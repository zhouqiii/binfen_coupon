<template>
  <div class="myOrder">
    <!-- 头部导航 -->
    <van-nav-bar @click-left="onClickLeft" class="headTicket" title="优惠券订单" left-arrow  :fixed="true" :placeholder="true">
    </van-nav-bar>
    <!-- 优惠券订单 -->
    <div class="ticket">
        <van-tabs v-model="active" color="#F44336" line-width="60px" line-height="3px"
        title-active-color="#F44336" @click="cutNav">
            <van-tab 
            v-for="(it,index) in arr" :key="index" 
            :title="it.title"
            >
              <!-- 遮罩层 -->
              <div v-show="!isReq" class="reqMask"></div>
              <div class="errorPage" v-if="!network">
                <div class="errorPageImg">
                  <!-- 图片转成base64 -->
                  <img src="data:image/png;base64," />
                </div>
                <div class="errorPageWord">网络出现了问题，获取数据失败</div>
                <div class="reload" @click="loadAgain()">
                  <div class="reloadSvg">
                    <svg t="1611124548462" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1169" width="0.9375rem" height="0.9375rem"><path d="M511.997366 117.728395a392.928395 392.928395 0 0 1 233.797531 76.90535l-107.693827 15.27572a39.506173 39.506173 0 0 0 5.530864 78.485597 40.586008 40.586008 0 0 0 5.636214-0.263375l184.36214-26.337448a39.348148 39.348148 0 0 0 33.527572-44.510288l-26.337448-183.835391a39.506173 39.506173 0 0 0-78.222223 11.061728l10.53498 72.954733A472.678189 472.678189 0 0 0 62.575144 661.069959l64-64A393.112757 393.112757 0 0 1 511.997366 117.728395z m449.474897 244.674897l-64 64A394.482305 394.482305 0 0 1 273.985844 825.679012l85.57037-12.115226a39.506173 39.506173 0 1 0-11.167078-78.222222l-184.36214 26.337448a39.506173 39.506173 0 0 0-33.527572 44.510288l26.337448 183.835391A39.506173 39.506173 0 0 0 195.947984 1024a41.08642 41.08642 0 0 0 5.636214-0.526749 39.242798 39.242798 0 0 0 33.527572-44.510288l-13.537449-94.55144a472.546502 472.546502 0 0 0 739.897942-522.008231z" p-id="1170" fill="#F44336"></path></svg>
                  </div>  
                  <div>重新加载</div>
                </div>
              </div>
              <van-list
                  v-else
                  v-model="loading"
                  :immediate-check="false"
                  :offset=50
                  :finished="finished"
                  finished-text=""
                  :error.sync="error"
                  error-text=""
                  @load="cutNav('kong',it.title)"
                >
                <div v-if="error" class="errorPage">
                  <!-- <div v-if="isFour"> -->
                    <div class="errorPageImg">
                      <img src="../assets/icon/404.png" />
                    </div>
                    <div class="errorPageWord">sorry,页面飞走了</div>
                  <!-- </div> -->
                </div>

                <div v-else-if="kongCouponList">
                  <div class="notyetphoto">
                      <img class="notyet-icon" src="@/assets/myOrder/notyet-order@2x.png"/>
                  </div>
                  <div class="notYet">暂无优惠券订单</div>
                </div>

                <ul v-else class="orderUl">
                  <li v-for="(item,index) in couponList" :key="2^index"
                  class="orderItem" @click="gotoAppItem(item.orderId,item.waresId,item.title,item.flag,item)">
                      <div  class="orderItem-left" v-lazy:background-image="gfrdDefultImg">
                          <img v-lazy="item.imgUrl" />
                      </div>
                      <div  class="orderItem-center">
                        <div>
                          <div class="name"><div class="nameFlow">{{item.name}}</div></div>
                          <div class="time">{{item.validatyBegin}}-{{item.validatyEnd}}</div>
                          <div v-if="item.title != '待支付' && item.state != 1 && item.state != 3 && item.state != 6" class="finish"><span>购买完成&nbsp</span>{{item.payDate? item.payDate:item.orderDate}} {{item.payTime? item.payTime:item.orderTime}}</div>
                          <div v-if="item.state == 1" style="letter-spacing:0px;" class="finish">退款完成 {{item.returnDate}}</div>
                        </div>
                      </div>
                      <div class="orderItem-right">
                        <ul class="right-posi">
                            <li v-if="item.couponType == '01'" class="discount">
                              <div>
                                <div class="num">￥<span :class="{'control':item.voucherAmount.toString().length>4}">{{item.voucherAmount}}</span></div>
                                <div v-if="item.termsAmount && item.termsAmount!=0" 
                                class="coupon">满{{item.termsAmount}}可用</div>
                                <div v-else-if="item.termsAmount==0" class="coupon">无门槛券</div>
                              </div>
                            </li>
                            <li v-else-if="item.couponType == '02'" class="discount">
                              <div>
                                <div class="numb"><span>{{item.discountRate}}</span>折</div>
                                <div v-if="item.termsAmount && item.termsAmount!=0" class="coupon">
                                  满{{item.termsAmount}}可用</div>
                                <div v-else-if="item.termsAmount==0" class="coupon">无门槛券</div>
                              </div>
                            </li>
                            <li v-else class="discount">
                              <div class="electron">电子券</div>
                            </li>
                            <li class="state">
                              <!-- state:可使用0；已使用2；退款中3；退款成功1;退款失败6 ；可退货4；不可退货5-->
                              <div v-if="item.title == '待支付'" style="color:#fff;background:#F44336;">去支付</div>
                              <div v-else-if="item.state == 3" style="color:#FFA837;border:1px solid #FFA837;">退货中</div>
                              <div v-else-if="item.state == 1" style="color:#999;border:1px solid #999;">退货完成</div>
                              <div v-else-if="item.state == 6" style="color:#F44336;border:1px solid #F44336;">退货失败</div>
                              <div v-else style="color:#F44336;border:1px solid #F44336;">查看票券</div>
                            </li>
                          </ul>  
                      </div>
                  </li>
                </ul>
              </van-list>
            </van-tab>
        </van-tabs>
    </div>
  </div>
</template>
<script>
import Toast from "@/components/Toast";
import createDom from "@/utils/createDom"
export default {
  name:'myOrder',
  data () {
    return {
      active: 0,
      arr:[{
        title:'全部订单',
        content:'全部订单'
      },{
        title:'待支付',
        content:'待支付'
      },{
        title:'已支付',
        content:'已支付'
      },{
        title:'退货',
        content:'退货'
      }],
      // 默认图片
      gfrdDefultImg:require('../assets/myTicket/default-img@2x.png'),
      wifiIng:require('../assets/icon/wifi.png'),
      // 优惠券数据
      couponList:[],
      // 请求第几页的数据
      NextPage:1,
      // vant 下拉加载
      loading: false,
      finished: false,
      error: false,
      // 防止重复 下拉加载
      noRepeat:true,
      // 是否是我主动调起的登录
      myLogin:false,
      // 优惠券列表为空
      kongCouponList:false,
      // 判断 进入待支付详情页面时，是在待支付还是全部订单
      payorall:'all',
      // 判断是断网还是请求错误
      // isFour:true
      // 默认有网
      network:true,
      // 切换导航时 有网,默认有
      haveWifi:true,
      // 当上一个请求未完成时，禁止下一次请求
      isReq:true
    }
  },
  mounted(){
    if (navigator.onLine) {
      this.haveWifi = true
    } else {
      this.network = false
      this.haveWifi = false
    }
    window.addEventListener('online',()=>{
      this.haveWifi = true
      // console.log('')
    })
    window.addEventListener('offline',()=>{
      this.network = false
      this.haveWifi = false
      // console.log('已断网')
    })
    this.registMethod();
    this.loginInAfter()
    this.cutNav('全部订单','全部订单')
  },
  methods:{
    loadAgain(){
      window.location.reload()
    },
    /* 待支付：支付成功后 */
    registMethod() {
      let _this = this;
      //待支付：支付成功后 
      this.setupWebViewJavascriptBridge(function(bridge) {
        bridge.registerHandler("goPaySucess", function(data) {
          if(_this.notNull(data)) {
            if(typeof data === "string") {
              data = JSON.parse(data);
            }
            //去重标记
            // _this.isPayBtnClickAgin = true;
            let PaySucess = data.PaySucess;//PaySucess为字符串1
            let PayFail = data.PayFail;//PayFail为字符串1
            if(_this.notNull(PaySucess)) {//支付成功
              //刷新页面
              if(_this.payorall == 'all'){
                _this.cutNav('全部订单','全部订单')
              }else if(_this.payorall == 'pay'){
                _this.cutNav(1,'待支付')
              }
              
            } else if(_this.notNull(PayFail)){//支付失败
              //刷新页面
              if(_this.payorall == 'all'){
                _this.cutNav('全部订单','全部订单')
              }else if(_this.payorall == 'pay'){
                _this.cutNav(1,'待支付')
              }
            }
          }
        });
      });
    },
    /* 登录成功后 */
    loginInAfter(){
      let _this = this;
      this.setupWebViewJavascriptBridge(function(bridge) {
        bridge.registerHandler("loginIn", function(data) {
          if(_this.notNull(data)) {
            if(typeof data === "string") {
              data = JSON.parse(data);
            }

            let flag = data.flag;
            if(_this.notNull(flag)) {//登录成功
              //刷新页面
              if(_this.myLogin === true){
                _this.cutNav('全部订单','全部订单')//刷新页面
              }
              _this.myLogin = false
              
            } 
          }
        });
      });
    },
    // 返回至客户端
    onClickLeft(){
      this.$router.push({
          path: '/myOrderList',
      })
    },
    // 跳转至客户端 优惠券详情
    gotoAppItem(orderId,waresId,title,flag,item,/*name,total,price,accumPoints,validatyEnd*/){
      // console.log(orderId)
      if(title){//待支付 优惠券详情
        if(flag == 'ZCYL'){//注册有礼
          this.callAppMethod({
            callName:"goResigetOrder",
            parameters:item
            // parameters:{
            //   data:{
            //     couponName:name,          //（优惠券名称）
            //     orderId:orderId,          //（订单号）
            //     total:total,              //（购买数量）
            //     couponPrice:price,        //（优惠券价格）
            //     couponPoint:accumPoints,  //（优惠券积分）
            //     couponDate:validatyEnd    //（有效时间结束时间）
            //   }
            // }
          })
        }else{
          this.callAppMethod({
            callName:"goBannerModel",
            parameters:{
              uiJump:'1',
              uiJumpId:'200014',
              totalOrderId:orderId
            }
          })
        }
        
      }else{//其他 优惠券详情
        this.callAppMethod({
          callName:"goMyCouponDetail",
          parameters:{
            orderId:orderId,
            couponId:waresId,
            isAbroad:''
          }
        })
      }
      
    },
    // 切换导航
    cutNav(name,title){
      //如果点击导航，则清空
      if(name !== 'kong'){
        if(this.haveWifi === true){
          this.network = true
        }
        this.couponList = []
        this.NextPage = 1
        this.finished = false
        this.error = false
        this.kongCouponList = false
        // this.isFour = true
      }
      if(this.noRepeat == true){
        if(title === '全部订单'){
          if(this.NextPage === 1){
            this.tobePaid(title,'allorders')
          }else{
            this.getRequest()
          }
          this.payorall = 'all'
        }else if(title === '待支付'){
          this.payorall = 'pay'
          this.tobePaid(title)
        }else{
          this.getRequest(title)
        }
      }
        
      
    },
    // 处理日期格式
    handleDate(date,sign){
      let y = date.substr(0,4)
      let m = date.substr(4,2)
      let d = date.substr(6,2)
      date = y+sign+m+sign+d
      return date
    },
    // 处理时间格式
    handleTime(time){
      if(time.indexOf(':') == -1){
        let h = time.substr(0,2)
        let m = time.substr(2,2)
        let s = time.substr(4,2)  

        time = h+':'+m+':'+s
        return time
      }else{
        return time
      }
    },
     // 调用接口，请求数据
    getRequest(title){
      this.isReq = false
      this.loading = true
      this.noRepeat = false
      let _this = this
      //0：可使用 1：已使用2:已过期;4:已退货;5:已支付
      let state = ''
      if(title == '退货'){
        state = '4'
      }else if(title == '已支付'){
        state = '5'
      }
      
      this.request({
        url: '/couponsNew.do',
        method:'get',
        params: {
          txnId:this.isMobile() + 'BSW100002',
          call:'cgi',
          certType:'',// v 证件类型 IDCD 110105197207260390
          certId:'',// v 证件号 211303199601011628 110221199006169982 
          pageNo:this.NextPage,//页码
          state:state,//优惠券状态：1:已使用;2:已过期;3:退货;4:待支付;5:已支付
        },
        headers: {
          'Content-Type': 'application/json;charset=UTF-8',
          'dns': 'dsmsboc001',
        },
      }).then((res)=>{
        _this.isReq = true
        _this.loading = false//loading
        _this.noRepeat = true 
        if(res.stat == "00"){
          // console.log(res)
          let data = res.body

          if(data){
            data.couponList.forEach((item)=>{
              if(item.orderDate){
                item.orderDate = _this.handleDate(item.orderDate,'.')
              }
              if(item.payDate){
                item.payDate = _this.handleDate(item.payDate,'.')
              }
              if(item.orderTime){
                item.orderTime = _this.handleTime(item.orderTime)
              }
              if(item.payTime){
                item.payTime = _this.handleTime(item.payTime)
              }

              if(item.returnDate){
                item.returnDate = _this.handleDate(item.returnDate,'/')
              }
              item.validatyBegin = item.validatyBegin.replace(/-/g, '/')
              item.validatyEnd = item.validatyEnd.replace(/-/g, '/')
              if(item.discountRate){
                let discount = item.discountRate*10+''
                item.discountRate = discount.substr(0,3)
              }
              _this.couponList.push(item)
            })
          }
        
          if(data && data.hasNextPage>0){
            _this.NextPage++
          }else if(data && data.hasNextPage==0){
            // 当列表为空，显示暂无票券
            if(_this.couponList.length > 0){
              _this.finished = true
            }else{
              _this.finished = true
              _this.kongCouponList = true
            }
          }else{
            if(_this.NextPage == 1){
              _this.couponList = []
            }
            _this.error = true;
            // _this.isFour = true
          }
            
        }else if(res.stat == "01"){//如果 未登录
          _this.callAppMethod({
            callName:"toLogin"
          })
          _this.myLogin = true
          _this.finished = true
        }else{
          _this.finished = true
          _this.error = true;
          // _this.isFour = true
        }
      }).catch(err=>{
        _this.isReq = true
        _this.error = true;
        // _this.isFour = false;
        _this.loading = false//loading
        _this.noRepeat = true
      })
    },
    // 待支付
    tobePaid(title,allorders){
      this.isReq = false
      let _this = this
      this.loading = true
      this.noRepeat = false
      this.request({
        // url: 'https://alex188.cn/Coupons/newWaitPayList.do?',
        url: '/coupons.do',
        params: {// v 为必传参数
          // this.isMobile() 获取移动端设备 1：Android；2：ios
          txnId:this.isMobile()+'NEW100018',
          call:'newWaitPayList',
          currPage:0,//页码 '1'
          clientType:_this.isMobile(),// 设备
          certId:'',// v 证件号 11010519950803007X 110221199006169982
          certType:'',// v 证件类型 ICDC
          reqChannel:_this.channel,// v 渠道：B缤纷;T手机银行;W微信公众号;03华奥;00中银运动小程序
        },
      }).then((res)=>{
        _this.isReq = true
        _this.loading = false//loading
        _this.noRepeat = true
        if(res.stat == '00'){
          
          if(res.list){
            res.list.forEach((item)=>{
              item.title = '待支付'
              item.orderId = item.totalOrderId
              item.waresId = ''
              _this.couponList.push(item)
            })
          }
         
          if(res && res.hasNextPage>0){
            _this.NextPage++
          }else if(res && res.hasNextPage==0){
            if(title == '待支付'){//待支付
              if(_this.couponList.length == 0){
                _this.kongCouponList = true
                _this.finished = true
              }else{
                _this.finished = true
              }
            }else{//全部订单
              if(allorders === 'allorders'){
                _this.getRequest()
              }
            }
          }else{
            _this.error = true;
            // _this.isFour = true
          }
        }else if(res.stat == '11'){//查无数据，往下走
          if(allorders === 'allorders'){//全部订单
            _this.getRequest()
          }else{//待支付
            _this.kongCouponList = true
            _this.finished = true
          }
        }else if(res.stat == '01' || res.stat == '02'){//如果 未登录
          _this.callAppMethod({
            callName:"toLogin"
          })
          _this.myLogin = true
          _this.finished = true
        }else{
          _this.error = true;
          // _this.isFour = true
        }
      }).catch(err=>{
        _this.isReq = true
        _this.couponList = []
        _this.error = true;
        // _this.isFour = false;
        _this.loading = false//loading
        _this.noRepeat = true
      })

      // console.log(allorders)
        // let res = {
        //   stat: "11",
        //   date: "2020-12-09 16:26:10",
        //   result: "查无数据",
        // }
        // let res={
        //   date: "2020-12-09 16:26:10",
        //   hasNextPage: "0",
        //   result: "交易成功",
        //   stat: "00",
        //   list:[
        //     {
        //     accumPoints: "0",
        //     cardNo: "6227607521769360",
        //     icon: "http://22.188.227.137:9080/dsms/image/WYSWL617540.png",
        //     imgUrl: "http://22.188.227.137:9080/dsms/image/WYSWL617540.png",//couponImgUrl
        //     intfId: "2",
        //     name: "零一二三四五六七八九十零一二三四五六七八九十",//waresName
        //     payType: "1",
        //     price: "1.00",
        //     total: "1",
        //     totalOrderId: "201209160158631520",
        //     validatyBegin: "2019-03-13",//startTime
        //     validatyEnd: "2039-04-08",//endTime
        //     state:'7'
        //   }
        //   ]
        // }

        // res.list.forEach((item)=>{
        //   item.title = '待支付'
        //   item.orderId = item.totalOrderId
        //   _this.couponList.push(item)
        // })
        // if(res && res.hasNextPage>0){
        //   _this.NextPage++
        // }else if(res && res.hasNextPage==0){
        //   if(title == '待支付'){
        //     if(_this.couponList.length == 0){
        //     }else{
        //       _this.finished = true
        //     }
        //   }else{
        //     if(allorders === 'allorders'){
        //       _this.getRequest()
        //     }
        //   }
        // }else{
        //   _this.error = true;
        // }
      // console.log(_this.couponList)
    },
   
  }
}
</script>
<style lang="less" scoped>
.myOrder{
  min-height: 100vh;
  background-color: #F8F8F8;
}
.notyetphoto{
  width: 200px;
  height: 200px;
  // padding: 300px 0 24px;
  margin: 300px auto 24px;
  img{
    width: 100%;
    height: 100%;
  }
}
.notYet{
  text-align: center;
  color:#333;
  font-size: 30px;
}
.reqMask{
  position: fixed;
  top:0;
  width: 100vw;
  height: 100vh;
  z-index: 100;
}
.errorPage{
  background: #F8F8F8;
  position: fixed;
  top:0;
  width: 100vw;
  height: 100vh;
  .errorPageImg{
    width: 200px;
    height: 200px;
    margin: 534px auto 0;
    background: no-repeat;
    background-size: 200px 200px;
    img{
      width: 100%;
      height: 100%;
    }
  }
  .errorPageWord{
    margin-top: 24px;
    text-align: center;
    font-size: 30px;
    color: #333;
  }
  .reload{
    color: #F44336;
    font-size: 30px;
    margin-top: 27px;
    // text-align: center;
    display: flex;
    justify-content: center;
    .reloadSvg{
      margin-right: 11px;
      transform:rotate(-45deg)
    }
  }
}

.orderUl{
  padding: 32px 32px 0;
}
.orderItem{
  display:flex;
  height: 180px;
  margin-bottom: 24px;
  background: url('../assets/myTicket/ticket-back@2x.png') no-repeat;
  background-size:100% 100%;
}
.orderItem-left{
  width: 180px;
  height: 180px;
  background: no-repeat;
  background-size: 180px 180px;
  img{
    width: 100%;
    height: 100%;
    border-radius: 16px;
  }
}
.orderItem-center{
  flex:1;
  padding:0 20px 0 16px;
  display: flex;
  align-items: center;
  .name{
    font-weight: 600;
    font-size: 28px;
    line-height: 150%;
    color: #333;
    .nameFlow{
      // height: 96px;
      overflow : hidden;
      text-overflow: ellipsis;
      display: -webkit-box;
      -webkit-line-clamp: 2;
      -webkit-box-orient: vertical;
    }
  }
  .time{
   margin-top: 8px;
    color: #999;
    font-size: 22px;
  }
  .finish{
    margin-top: 16px;
    font-size: 24px;
    color: #666;
    letter-spacing:-1px;
    white-space:nowrap;
  }
}
.orderItem-right{
  width: 180px;
  // background-color: rgba(215, 215, 215, 1);
  // background-color:#fff;
  text-align: center;
  .right-posi{
    width: 100%;
    height: 180px;
    // position: relative;
    .discount{
      height: 64%;
      display: flex;
      justify-content: center;
      align-items: center;
      .numb{
        font-size: 40px;
        color: #F44336;
        font-weight: bold;
        margin-bottom: 16px;
      }
      .num{
        font-size: 32px;
        color: #F44336;
        font-weight: bold;
        span{
          font-size: 56px;
        }
        .control{
          font-size: 36px;
        }
      }
      .coupon{
        font-size: 24px;
        color: #999;
        margin-top: 8px;
      }
      .electron{
        color: #F44336;
        font-weight: bold;
        font-size: 40px;
      }
    }
    .state{
      width: 152px;
      line-height: 48px;
      margin: 0 auto;
      font-size: 26px;
      div{
        border-radius: 8px;
      }
    }
  }
}


.headTicket{
    // 修改头部导航icon的颜色
    /deep/ .van-nav-bar .van-icon{
        color:#444;
    }
    /deep/ .van-nav-bar .van-nav-bar__content{
        height: 92px;
        .van-nav-bar__title{
            font-size: 36px;
            line-height: 150%;
            font-weight: bold;
        }
        .van-nav-bar__arrow{
            font-size: 44px;
            line-height: 100%;
        }
    }
}
// 修改tab表签的默认样式
.ticket{
    /deep/ .van-tabs__wrap{
        height: 88px;
        position: fixed;
        width: 100%;
        z-index: 1;
    }
    /deep/ .van-tabs__nav--line{
        .van-tab{
            font-size: 30px;
            line-height: 150%;
            color: #999;
        }
    }
    // 内容
    /deep/  .van-tabs__content{
        padding: 88px 0 24px;
        .van-list__loading{//加载中...  的样式
          .van-loading{
            span:nth-child(1){
              width: 28px !important;
              height: 28px !important;
            }
            .van-loading__text{
              font-size:28px;
            }
          }
        }
    }
}
</style>
